...
export const PhotoEditor: React.FC = () => {
    ...
    React.useEffect(() => {
        // Use relative path - handled by Vercel Rewrites (prod) or Vite Proxy (dev)
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            ctx.font = `${cityFontSize}px DreamPlanner, sans-serif`;
            const cityText = (cityName || 'YOUR CITY').toUpperCase();
            ctx.fillText(cityText, 0, 0);

            // "CAMPFIRE" (Top Line)
            const campfireFontSize = w * 0.12;
            ctx.font = `${campfireFontSize}px DreamPlanner, sans-serif`;
                    ...
                    <div className="relative">
                        ...
                        >
                            <option value="" disabled>
                                {loading ? "Loading campfires..." : "Select a location..."}
                            </option>
                            {campfires.map((campfire) => (
                                <option key={campfire.id} value={campfire.eventName}>
                                    {campfire.eventName}
